...
// Named rather than default: pino-http is CommonJS, so its default binding is
// the whole module object, which is not callable.
import { pinoHttp, type Options } from 'pino-http';

import { logger } from '../config/logger.js';

/**
 * Either shape this module sees a request in: the raw one Express passes to the
 * message callbacks, or pino's serialised form.
 */
type LoggedRequest = { url?: string; originalUrl?: string };

/** The URL as the client asked for it, before Express rewrote it for a router. */
const reqUrl = (req: LoggedRequest): string => req.originalUrl ?? req.url ?? '';
 ...
 */
export const requestLoggerOptions: Options<IncomingMessage, ServerResponse> = {
  ...
  customLogLevel: (req, res, err): Level => {
  ...
  // pino-http wraps custom serializers by default, so this one is handed
  // pino's own serialised request rather than the raw IncomingMessage: `url`
  // is already the original URL and `remoteAddress` has been lifted off the
  // socket.
  serializers: {
 ...
 */
export const createRequestLogger = (instance: Logger = logger) =>
