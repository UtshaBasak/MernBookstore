...
const log = (...args) => console.log('[migrate:images]', ...args);
...
export const migrateImages = async ({ dryRun = false, limit = 0 } = {}) => {
  ...
  for (const book of candidates) {
    ...
    try {
      ...
      summary.failed += 1;
      log(`  FAILED "${book.title}": ${error.message}`);
    }
  }
...
const main = async () => {
  ...
  log(`connected to ${config.mongoUri.replace(/\/\/[^@]*@/, '//***@')}`);
...
const invokedDirectly = process.argv[1]?.replace(/\\/g, '/').endsWith('scripts/migrateImages.js');
if (invokedDirectly) {
