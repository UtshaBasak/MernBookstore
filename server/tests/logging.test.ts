...
describe('what gets logged', () => {
   ...
   */
  /** One serialised log line, as the configured serializers shape it. */
  ...
  const capture = async ({ url, originalUrl, statusCode = 200 }: CaptureOptions) => {
    ...
    const capturing = pino({ level: 'info' }, { write: (l: string) => void lines.push(JSON.parse(l)) });
    ...
    // Hand-built stand-ins for a request and response: enough of each for the
    // middleware to run, which is far less than the full Node declarations.
  ...
  it('records the client address', async () => {
    const lines = await capture({ url: '/', originalUrl: '/book' });
