...
/** The module under test, re-imported after the environment is set. */
type CloudinaryModule = typeof import('../config/cloudinary.js');

describe('signature generation', () => {
  const withCredentials = async (fn: (mod: CloudinaryModule) => Promise<void> | void) => {
    vi.resetModules();
...
describe('URL ownership', () => {
  const withCloud = async (fn: (mod: CloudinaryModule) => Promise<void> | void) => {
    vi.resetModules();
    process.env.CLOUDINARY_CLOUD_NAME = 'testcloud';
