...
import { createTestContext, clearDatabase, closeTestContext, type PrefixedRequest, setCookies } from './helpers/testApp.js';
 ...
 *
 * Throws rather than returning null: every caller here has just signed in or
 * refreshed, so a missing cookie is the test failing, not a case to handle.
 */
const refreshCookie = (res: Response): string => {
  const found = setCookies(res).find((c) => c.startsWith('refreshToken='));
  if (!found) throw new Error('No refreshToken cookie on the response');
...
describe('sign-in issues both tokens', () => {
  ...
  it('marks the cookie httpOnly and scopes it to the auth routes', async () => {
    ...
    const raw = setCookies(res).find((c) => c.startsWith('refreshToken='));
...
describe('logout', () => {
  it('ends the session and clears the cookie', async () => {
    ...
    const cleared = setCookies(res).find((c) => c.startsWith('refreshToken='));
